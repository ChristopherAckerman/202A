...
\begin{lstlisting}[caption={Matlab output from AR(1) model on data}, captionpos=b]
...
\begin{lstlisting}[caption={Matlab output from AR(1) model on residuals}, captionpos=b]
...
\label{fig:q4-autocorrplot}
\end{figure}


This is a great model. The value of the autoregressive coefficient is highlgy significant.
The R squared is \( 0.999857 \). Since we are estimating only 1 lag the
adjusted R squared is very similar.

Notice that the autocorrelation graph for the sample residuals show that most
of the lags are within the confidence intervals around 0 (with the exception of
the 2nd lag.) I'd say that shows that the model fits the data pretty well. The
is zero and the there is likely 0 to no autocorrelation so the residuals are
indeed whitenoise.

I suppose that I could also see if the adjusted R squared goes up or down if
I put a 2nd lag in the model (see Hall's first few paragraphs on how there is
no more predictive power in adding more lags) but I feel like that would be
